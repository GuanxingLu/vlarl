...
class Args:
    ...
    """Length of the query"""
    number_envs_per_task_id: int = 2
    """the number of envs for each task id (group)"""
...
def calculate_runtime_args(args: Args,):
    ...
    args.num_groups = exact_div(args.local_rollout_batch_size, args.number_envs_per_task_id)

    # check if the task id in the same group are the same
    # e.g., task_ids = [0, 0 | 1, 1], number_envs_per_task_id = 2, num_groups = 2
    task_ids = torch.tensor(args.task_ids)
    for i in range(args.num_groups):
        assert (task_ids[i * args.number_envs_per_task_id : (i + 1) * args.number_envs_per_task_id] == task_ids[i * args.number_envs_per_task_id]).all(), \
            f"{task_ids[i * args.number_envs_per_task_id : (i + 1) * args.number_envs_per_task_id]=}, {task_ids[i * args.number_envs_per_task_id]=}"
    del task_ids
...
class PolicyTrainerRayProcess(RayProcess):
    ...
    def train(
        ...
        # args.max_env_length = args.num_steps    # NOTE: for reinforce, we make sure the env is always done
        ...
        values = torch.zeros(
            ...
            device=device,
            dtype=torch.float32,
        )   # track the moving average of the return as a baseline
        actions = None
        ...
        for training_step in range(resume_training_step, args.num_training_steps):
            ...
            for step in range(args.num_steps):
                ...
                # Create mask for active (not done) environments
                active_mask = ~dones_next.bool()
                active_indices = torch.nonzero(active_mask).squeeze(-1)
                
                if len(active_indices) > 0:  # Only step active environments
                    # Get the actual rollout indices for active environments
                    active_rollout_indices = active_indices.cpu().numpy()
                    
                    # Only select actions for active environments
                    active_actions = local_actions[active_indices.cpu().numpy()]
                    active_log_probs = vllm_logprobs[step][active_indices].detach().cpu().numpy()
                    
                    with timer.timer("env_step"):
                        # Step only active environments
                        active_obs, active_rewards, active_dones, active_infos = train_envs.step(
                            active_actions,
                            log_probs=active_log_probs,
                            id=active_rollout_indices,
                        )
                    ...
                    for i, idx in enumerate(active_indices.cpu().numpy()):
                        local_obs["prompts"][idx] = active_obs["prompts"][i]
                ...
                local_rewards = torch.tensor(local_rewards, device=device, dtype=torch.float32) if not isinstance(local_rewards, torch.Tensor) else local_rewards
                local_dones = torch.tensor(local_dones, device=device, dtype=torch.float32) if not isinstance(local_dones, torch.Tensor) else local_dones
            ...
            # GRPO LOGIC: compute group rewards instead of value model output
            # Compute advantages as (reward - mean_reward) / std_reward within each group
            mean_grouped_returns = torch.zeros(args.num_steps, args.num_groups, device=scores.device)
            std_grouped_returns = torch.zeros(args.num_steps, args.num_groups, device=scores.device)
            advantages = torch.zeros_like(scores)
            
            # Mark all as done at the final step to ensure proper advantage computation
            dones[args.num_steps - 1, :] = True
            
            with torch.no_grad() and timer.timer("advantage"):
                # Compute group statistics for each step
                for t in range(args.num_steps):
                    for i in range(args.num_groups):
                        start_idx = i * args.number_envs_per_task_id
                        end_idx = (i + 1) * args.number_envs_per_task_id
                        group_rewards = scores[t, start_idx:end_idx]
                        
                        mean_grouped_returns[t, i] = group_rewards.mean()
                        std_grouped_returns[t, i] = group_rewards.std() + 1e-8  # Add small epsilon to avoid division by zero
                        advantages[t, start_idx:end_idx] = (group_rewards - mean_grouped_returns[t, i]) / std_grouped_returns[t, i]
