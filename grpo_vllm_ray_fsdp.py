...
class Args:
    ...
    """Whether to automatically reset the environment when done (set to False for GRPO in LIBERO)"""
    ...
    """the number of envs in a group"""
    group_size: Optional[int] = None
    local_num_groups: int = 8
    ...
    nonneg_adv: bool = False
    """set all negative advantages to zero"""
    clip_ploss: bool = True
    """whether to use clipped policy loss"""
...
def calculate_runtime_args(args: Args,):
    ...
    args.local_rollout_batch_size = args.local_num_groups * args.local_group_size
    if args.local_rollout_forward_batch_size is None:
        args.local_rollout_forward_batch_size = args.local_rollout_batch_size
    ...
    args.group_size = int(args.local_group_size)
    args.num_groups = int(args.local_group_size * args.world_size)
    ...
    args.task_ids = args.task_ids * args.rollout_batch_size
...
def get_environment(args: Args, mode: str = "train"):
    env = LiberoVecEnv(
        ...
        group_size=args.local_group_size,
        num_group=args.local_num_groups,
    )
    ...
    if mode == "train" and args.save_video:
        ...
        env = VideoWrapper(env, save_dir=save_dir, rank=args.env_gpu_id)
            ...
            for step in range(args.num_steps):
                ...
                with torch.no_grad():
                    ...
                    for i in range(0, args.local_rollout_batch_size, args.local_rollout_forward_batch_size):
                        ...
                        # NOTE: GRPO doesn't use value function, so we skip value computation
                ...
                # logger.info(f"{local_rewards=}, {local_dones=}")
                ...
                for epoch_idx in range(args.num_epochs):
                    ...
                    for mini_batch_start in range(
                        ...
                        for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                            ...
                            logger.info(f"{mb_advantage=}")
                            logger.info(f"{mb_logprobs=}, {new_logprobs=}, {ratio=}")

                            # Compute policy loss with optional clipping
                            if args.clip_ploss:
                                pg_losses = -mb_advantage * ratio
                                pg_losses2 = -mb_advantage * torch.clamp(ratio, 1.0 - args.cliprange_low, 1.0 + args.cliprange_high)
                                pg_loss = torch.max(pg_losses, pg_losses2).mean()
                            else:
                                pg_loss = (-mb_advantage * ratio).mean()

                            # Add entropy bonus for exploration
                            total_loss = pg_loss
                            if args.entropy_bonus > 0:
                                prob_dist = torch.nn.functional.softmax(new_logits, dim=-1)
                                entropy = torch.logsumexp(new_logits, dim=-1) - torch.sum(prob_dist * new_logits, dim=-1)
                                entropy_loss = -entropy.mean()  # negative because we want to maximize entropy
                                total_loss = pg_loss + args.entropy_bonus * entropy_loss
                        ...
                        minibatch_idx += 1
                        del mb_advantage, mb_responses, mb_query_responses, mb_return
                        del new_logprobs, logprobs_diff, ratio, pg_loss
                        if args.clip_ploss:
